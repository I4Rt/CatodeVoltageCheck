...
def crc16_reversed(data: bytes) -> bytes:
    """Calculate CRC-16 for Modbus."""
    crc_high = 0xFF
    crc_low = 0xFF
    ...
    for byte in data:
        ...
        crc_high = crc_low ^ HIGH_BYTES[index]
        crc_low = LOW_BYTES[index]
    return bytearray([crc_low, crc_high])

def add_crc(package: bytes, reversed = False) -> bytearray:
    """Add CRC-16 to bytes package."""
    if reversed:
        return bytearray(package) + crc16_reversed(package)
    else:
